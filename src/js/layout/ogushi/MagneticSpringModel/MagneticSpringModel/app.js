...
function setGraphLocation(graph) {
    ...
    function calculateLocationWithForceDirectedMethod(graph, medgelist, mfields) {
        ...
        var D = Math.min(WIDTH, HEIGHT) / 4; //スプリングの自然長
        var cs = 200; //スプリング力の係数
        var cr = 10; //斥力の係数
        var cm = 0.1; //磁力の係数
        ...
        //2点間の距離を計算する
        function length_dots(dot1, dot2) {
            var xl = dot1.x - dot2.x;
            var yl = dot1.y - dot2.y;
            return Math.sqrt(xl * xl + yl * yl);
        }
        //2点を結ぶ辺があればtrue、無ければfalseを返す
        function isEdgeConnectingTwoDots(dot1, dot2, edges) {
            var bool = false;
                        ...
                        if (length != 0) {
                            if (edgeConnectingTwoDots(dot1, dot2, edges) == null) {
                                ...
                                if (e.type == 0) {
                                    ...
                                    var mvec = mf.vector;
                                    var trad = dispartyAngle(mvec.angle, rad);
                                    if (trad >= 0) {
