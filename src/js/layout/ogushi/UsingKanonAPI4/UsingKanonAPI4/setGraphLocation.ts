...
function setGraphLocation(graph: Graph) {
    ...
    let EdgeWithPrimitiveValue: boolean = true;     //参照先がprimitive型のときに角度を決定するかどうか
    ...
    let STANDARD_EDGELENGTH: number = 130;
    ...
    edgeListInit(graph, FiFA_NodeList, FiFA_EdgeList, FiFA_ClusterNodeList, FiFA_ClusterEdgeList, classAndFieldList,
        DrawCircle, EdgeWithPrimitiveValue, attentionNodes);
    ...
    function edgeListInit(graph: Graph, nodelist: FiFA_Node[], edgelist: FiFA_Edge[],
        ...
        drawcircle: boolean, edgewithprimitivevalue: boolean, attentionNodes: string[]) {
        ...
        let allObjectClass: string[] = new Array(ObjectIDs.length);
        for (let i = 0; i < ObjectIDs.length; i++) {
        ...
        for (let i = 0; i < ObjectIDs.length; i++) {
            ...
            for (let j = 0; j < fields.length; j++) {
                ...
                let node2: FiFA_Node = nodelist[ObjectIDs.indexOf(ID2)];
        ...
        for (let i = 0; i < attentionNodes.length; i++) {
            ...
            nodelist[ObjectIDs.indexOf(attentionNodes[i])].color = pink;
        }
        ...
        if (attentionNodes.length > 0) {  //もし注目点があるのならば
            ...
            for (let i = 0; i < attentionNodes.length; i++) {
                let attendNode: FiFA_Node = nodelist[ObjectIDs.indexOf(attentionNodes[i])];
        ...
        for (let i = 0; i < clusteredgelist.length; i++) {
            if (clusteredgelist[i].cluster2.main != null) {
                if (clusteredgelist[i].cluster2.main.isLiteral) {
                    clusteredgelist[i].id_length *= 0.7;
                    clusteredgelist[i].edge.id_length *= 0.7;
                } else if (clusteredgelist[i].cluster1.main != null) {
    ...
    function makeMinimalNode(nodelist: FiFA_Node[], edgelist: FiFA_Edge[]): FiFA_Node[][] {
        ...
        for (let i = 0; i < edgelist.length; i++) {
            if (!edgelist[i].dot2.interested) {     //エッジのtoノードが興味なしのとき
                ...
                if (!edgelist[i].dot1.interested) {     //エッジのfromノードにのみ興味なしのとき
                    if (edgelist[i].dot2.isLiteral) {
                        edgelist[i].dot2.interested = false;
                        notInterestedNodes.push(edgelist[i].dot2);
                        notInterestedEdges.push(edgelist[i]);
                        edgelist[i].dot2.size = NODESIZE / 8;
                        edgelist[i].id_length = edgelist[i].id_length / 32;
                        edgelist[i].smooth = false;
                    }
                }
    ...
    function calculateLocationWithForceDirectedMethod(graph: Graph, nodelist: FiFA_Node[], edgelist: FiFA_Edge[],
        ...
        function stopCalculate() {
            ...
            for (let i = 0; i < ObjectIDs.length; i++) {
                graph.setLocation(ObjectIDs[i], nodelist[i].x, nodelist[i].y);      //ノードの座標
                graph.setDistance(ObjectIDs[i], nodelist[i].route_length);      //ノードの注目点からの距離（デバッグ用）
                graph.setSize(ObjectIDs[i], nodelist[i].size);                  //ノードのサイズ
                graph.setColor(ObjectIDs[i], nodelist[i].color);
            }

            for (let i = 0; i < edgelist.length; i++) {

                let edge: FiFA_Edge = edgelist[i];
                let dot1ID: string = ObjectIDs[nodelist.indexOf(edge.dot1)];
                let dot2ID: string = ObjectIDs[nodelist.indexOf(edge.dot2)];
            ...
            for (let i = 0; i < greenEdges.length; i++) {
                if (!nodelist[ObjectIDs.indexOf(greenEdges[i].to)].interested) {
                    let toNode: FiFA_Node = nodelist[ObjectIDs.indexOf(greenEdges[i].to)];
            ...
            if (attentionNodes.length > 0) {
                for (let i = 0; i < attentionNodes.length; i++) {
                    let node: FiFA_Node = nodelist[ObjectIDs.indexOf(attentionNodes[i])];
