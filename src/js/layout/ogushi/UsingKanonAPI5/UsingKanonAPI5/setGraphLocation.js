...
function setGraphLocation(graph) {
    ...
    var FiFA_Node = /** @class */ (function () {
        function FiFA_Node(ID) {
            ...
            var fieldnames = graph.getFields(ID);
            for (var i_1 = 0; i_1 < fieldnames.length; i_1++) {
                var fieldObjectID = graph.getField(ID, fieldnames[i_1]);
                if (graph.isLiteral(fieldObjectID)) {
                    this.literals[fieldnames[i_1]] = graph.nodes[fieldObjectID].value;
                }
            }
        }
        //座標を初期化する
    ...
    var FiFA_Cluster_Node = /** @class */ (function () {
        function FiFA_Cluster_Node(main, sub) {
            ...
            for (var i_2 = 0; i_2 < this.sub.length; i_2++) {
                this.sub[i_2].register_cluster(this);
            }
        }
        //初期化
        ...
        FiFA_Cluster_Node.prototype.sub_init = function (width, height) {
            for (var i_3 = 0; i_3 < this.sub.length; i_3++) {
                this.sub[i_3].init(this.x - width / 2 + Math.floor(Math.random() * width), this.y - height / 2 + Math.floor(Math.random() * height));
            }
        };
        //引数のノードが含まれているか判定する
        ...
        FiFA_Cluster_Node.prototype.move_only_node = function () {
            ...
            for (var i_5 = 0; i_5 < this.sub.length; i_5++) {
                this.sub[i_5].move(this.x - rx, this.y - ry);
            }
        };
        //ノードに働く力から速度を求める
    ...
    var UsingObjectIDs = new Array();
    var WIDTH = 1280; //表示する画面の横幅
    var HEIGHT = 720; //表示する画面の縦幅
    var NODESIZE = 15;
    var STANDARD_EDGELENGTH = 200;
    ...
    edgeListInit(graph, FiFA_NodeList, FiFA_EdgeList, FiFA_ClusterNodeList, FiFA_ClusterEdgeList, classAndFieldList, DrawCircle, attentionNodes);
    ...
    function sameClassAndField_InArray(caf, arrayCaf) {
        var index = -1;
        for (var i_6 = 0; i_6 < arrayCaf.length; i_6++) {
            if (caf.parentcls == arrayCaf[i_6].parentcls && caf.childcls == arrayCaf[i_6].childcls && caf.field == arrayCaf[i_6].field) {
                index = i_6;
            }
        }
        return index;
    ...
    function arraySpliceBoforeIndexOf(key, array) {
        ...
        function copyArray(origin) {
            ...
            for (var i_7 = 0; i_7 < origin.length; i_7++) {
                array[i_7] = origin[i_7];
            }
            return array;
        }
    }
     ...
     * グラフエッジの生成
     * ********************/
    function edgeListInit(graph, nodelist, edgelist, clusternodelist, clusteredgelist, caflist, drawcircle, attentionNodes) {
        var edgeListInitStartTime = performance.now();
        //ノード群の生成
        for (var i_8 = 0; i_8 < ObjectIDs.length; i_8++) {
            if (!graph.isLiteral(ObjectIDs[i_8])) {
                var fnode = new FiFA_Node(ObjectIDs[i_8]);
                fnode.interested = graph.notInterestedClass.indexOf(fnode.cls) == -1;
                fnode.attention = attentionNodes.indexOf(ObjectIDs[i_8]) != -1;
                nodelist.push(fnode);
                UsingObjectIDs.push(ObjectIDs[i_8]);
            }
        }
        //グラフ内で使われているオブジェクトのクラス名の配列
        var allObjectClass = new Array(UsingObjectIDs.length);
        for (var i_9 = 0; i_9 < UsingObjectIDs.length; i_9++) {
            allObjectClass[i_9] = graph.getClass(UsingObjectIDs[i_9]);
        }
        //重複を除いたクラス名の配列
        ...
        var _loop_1 = function (i_10) {
            IDsSeparateClass[i_10] = UsingObjectIDs.filter(function (value, index, array) {
                return graph.getClass(value) == allObjectClassExceptDuplication[i_10];
            });
        };
        for (var i_10 = 0; i_10 < allObjectClassExceptDuplication.length; i_10++) {
            _loop_1(i_10);
        }
        //エッジ群の生成
        for (var i_11 = 0; i_11 < UsingObjectIDs.length; i_11++) {
            //ID1(始点ノード)のIDとクラス
            var ID1 = UsingObjectIDs[i_11];
            //ID1の持つフィールドの列
            ...
            for (var j_1 = 0; j_1 < fields.length; j_1++) {
                ...
                if (!graph.isLiteral(ID2)) {
                    var node2 = nodelist[UsingObjectIDs.indexOf(ID2)];
                    var fedge = new FiFA_Edge(nodelist[i_11], node2, fieldname);
                    edgelist.push(fedge);
                    nodelist[i_11].f_edges.push(fedge);
                }
            }
        }
        //必要なフィールド名
        for (var i_12 = 0; i_12 < edgelist.length; i_12++) {
            var caf = new ClassAndField(edgelist[i_12].ID1class, edgelist[i_12].ID2class, edgelist[i_12].name, edgelist[i_12]);
            var index = sameClassAndField_InArray(caf, caflist);
            if (index == -1) {
                caflist.push(caf);
            }
            else {
                caflist[index].f_edges.push(edgelist[i_12]);
            }
        }
        necessaryCaFList(graph, caflist, UsingObjectIDs);
        //必要なフィールド名以外のエッジを削除する
        for (var i_13 = edgelist.length - 1; i_13 >= 0; i_13--) {
            var bool = false;
            for (var j_2 = 0; j_2 < caflist.length; j_2++) {
                bool = bool || edgeIncludeCaF(edgelist[i_13], caflist[j_2]);
            }
            if (bool == false) {
                edgelist.splice(i_13, 1);
            }
        }
        ...
        for (var i_14 = 0; i_14 < caflist.length; i_14++) {
            for (var j_3 = 0; j_3 < caflist[i_14].f_edges.length; j_3++) {
                caflist[i_14].f_edges[j_3].id_angle = caflist[i_14].angle;
            }
        }
        //注目ノードの色を変更する
        for (var i_15 = 0; i_15 < attentionNodes.length; i_15++) {
            var pink = void 0;
            if (graph.getClass(attentionNodes[i_15]) == "Kanon-ArrayNode") {
                pink = {
                    border: 'deeppink',
                ...
                pink = "hotpink";
            nodelist[UsingObjectIDs.indexOf(attentionNodes[i_15])].color = pink;
        }
        //極小ノードを生成する
        ...
        for (var i_16 = 0; i_16 < cluster_shortest_path.length; i_16++) {
            if (cluster_shortest_path[i_16] != clusternodelist.length && cluster_shortest_path[i_16] > shortest_path_max) {
                shortest_path_max = cluster_shortest_path[i_16];
            }
        }
        //各グローバル変数の値の計算
        ...
        if (attentionNodes.length > 0) { //もし注目点があるのならば
            ...
            for (var i_17 = 0; i_17 < attentionNodes.length; i_17++) {
                var attendNode = nodelist[UsingObjectIDs.indexOf(attentionNodes[i_17])];
            ...
            for (var i_18 = 0; i_18 < clusternodelist.length; i_18++) {
                ...
                for (var j_4 = 0; j_4 < indexes.length; j_4++) {
                    if (minLength > cluster_shortest_path[indexes[j_4] * clusternodelist.length + i_18]) {
                        minLength = cluster_shortest_path[indexes[j_4] * clusternodelist.length + i_18];
                    }
                }
                if (minLength != clusternodelist.length) {
                    clusternodelist[i_18].route_length = minLength;
                    if (maxDistance < clusternodelist[i_18].route_length) {
                        maxDistance = clusternodelist[i_18].route_length;
                    }
                }
            }
            //注目点との経路長を元に注目点との理想距離を計算
            for (var i_19 = 0; i_19 < clusternodelist.length; i_19++) {
                if (clusternodelist[i_19].route_length == 0) {
                    clusternodelist[i_19].feye_distance = 0;
                }
                else {
                    clusternodelist[i_19].feye_distance = (DISTORTION + 1) * maxDistance /
                        (DISTORTION + maxDistance / clusternodelist[i_19].route_length);
                }
            }
            for (var i_20 = 0; i_20 < clusteredgelist.length; i_20++) {
                clusteredgelist[i_20].feye_length(maxDistance, DISTORTION);
                clusteredgelist[i_20].edge.id_length = clusteredgelist[i_20].id_length;
            }
            //console.log("edges =");
            //console.log(edges);
            //注目点との距離を元に各点のサイズを追加
            for (var i_21 = 0; i_21 < clusternodelist.length; i_21++) {
                if (clusternodelist[i_21].route_length != -1 && maxDistance != 0) {
                    clusternodelist[i_21].size = NODEMAXSIZE - (NODEMAXSIZE - NODEMINSIZE) * clusternodelist[i_21].feye_distance / maxDistance;
                }
                if (clusternodelist[i_21].main != null)
                    clusternodelist[i_21].main.size *= clusternodelist[i_21].size / NODESIZE;
                for (var j_5 = 0; j_5 < clusternodelist[i_21].sub.length; j_5++) {
                    clusternodelist[i_21].sub[j_5].size *= clusternodelist[i_21].size / NODESIZE;
                }
                for (var j_6 = 0; j_6 < clusternodelist[i_21].f_include_edges.length; j_6++) {
                    clusternodelist[i_21].f_include_edges[j_6].id_length *= clusternodelist[i_21].size / NODESIZE;
                }
            }
        }
        //プリミティブ型や配列型を参照しているエッジの理想長を短くする
        for (var i_22 = 0; i_22 < clusteredgelist.length; i_22++) {
            if (clusteredgelist[i_22].cluster2.main != null) {
                if (clusteredgelist[i_22].cluster1.main != null) {
                    if (clusteredgelist[i_22].cluster1.main.cls == "Kanon-ArrayNode" && clusteredgelist[i_22].name == "ref") {
                        clusteredgelist[i_22].id_length *= 0.7;
                        clusteredgelist[i_22].edge.id_length *= 0.7;
                    }
                }
            }
        }
     ...
     */
    function necessaryCaFList(graph, caflist, UsingObjectIDs) {
        for (var i_23 = caflist.length - 1; i_23 >= 0; i_23--) {
            var caf1 = caflist[i_23];
            //console.log("i = " + i);
            //console.log(caf1);
        ...
        function isOverlapping(graph, cafx, cafy) {
            var bool = true;
            for (var i_24 = 0; i_24 < UsingObjectIDs.length; i_24++) {
                var ID1 = UsingObjectIDs[i_24];
    ...
    function searchCycleGraph(graph, edgelist, IDs, arrayField) {
        ...
        for (var i_25 = 0; i_25 < cycleIDs.length; i_25++) {
            for (var j_9 = 0; j_9 < cycleIDs[i_25].length - 1; j_9++) {
                for (var k = 0; k < edgelist.length; k++) {
                    if (cycleIDs[i_25][j_9] == edgelist[k].ID1 && cycleIDs[i_25][j_9 + 1] == edgelist[k].ID2) {
        ...
        function cycleGraphIDs(graph, IDs, arrayField) {
            ...
            for (var i_26 = 0; i_26 < IDs.length; i_26++) {
                if (usedIDs.indexOf(IDs[i_26]) == -1) {
                    var cycleIDsFromOneID = cycleGraphIDsFromOneID(graph, usedIDs, arrayField, IDs[i_26]);
            ...
            function cycleGraphIDsFromOneID(graph, usedIDs, arrayField, ID) {
                ...
                function deep_first_search(graph, stack, cycleIDs, usedIDs, arrayField, nowID) {
                    ...
                    for (var i_27 = 0; i_27 < arrayField.length; i_27++) {
                        var u = graph.getField(nowID, arrayField[i_27].field);
                        if (u != undefined) {
    ...
    function decitionFieldAngle(caflist) {
        ...
        for (var i_28 = 0; i_28 < checklist.length; i_28++) {
            checklist[i_28] = -1;
        }
        for (var i_29 = 0; i_29 < caflist.length; i_29++) {
            if (checklist[i_29] == -1) {
                var cafnumber = 1;
                checklist[i_29] = 1;
                var from = caflist[i_29].parentcls;
                var to = caflist[i_29].childcls;
                if (from == "Kanon-ArrayNode" && to == "Kanon-ArrayNode") { //配列ノードの場合
                    if (caflist[i_29].field == "next") {
                        caflist[i_29].angle = 0;
                    }
                    else if (caflist[i_29].field == "ref") {
                        caflist[i_29].angle = 90;
                    }
                    checklist[i_29] = 0;
                }
                else if (from == to) { //フィールドの指すクラスが元のクラスと同じ場合
                    for (var j_11 = i_29 + 1; j_11 < caflist.length; j_11++) {
                    ...
                    var ii = 0;
                    for (var j_12 = i_29; j_12 < caflist.length; j_12++) {
                        if (checklist[j_12] == 1) {
                            if (cafnumber == 1) {
                    ...
                    for (var j_13 = i_29 + 1; j_13 < caflist.length; j_13++) {
                        if (caflist[j_13].parentcls == from && caflist[j_13].childcls == to) {
                            cafnumber++;
                            checklist[j_13] = 1;
                        }
                    }
                    var ii = 0;
                    for (var j_14 = i_29; j_14 < caflist.length; j_14++) {
                        if (checklist[j_14] == 1) {
                            caflist[j_14].angle = 180 - 180 / (cafnumber * 2) * (2 * ii + 1);
                            ii++;
                            checklist[j_14] = 0;
                        }
                    }
    ...
    function makeMinimalNode(nodelist, edgelist) {
        ...
        for (var i_30 = 0; i_30 < nodelist.length; i_30++) {
            if (!nodelist[i_30].interested) { //興味のないノードは
                nodelist[i_30].size = nodelist[i_30].size / 8; //大きさを小さくする
                notInterestedNodes.push(nodelist[i_30]);
            }
        }
        for (var i_31 = 0; i_31 < edgelist.length; i_31++) {
            if (!edgelist[i_31].dot2.interested) { //エッジのtoノードが興味なしのとき
                if (!edgelist[i_31].dot1.interested) { //エッジのfromノードも興味なしのとき
                    edgelist[i_31].id_length = edgelist[i_31].id_length / 32;
                    notInterestedEdges.push(edgelist[i_31]);
                }
                else {
                    edgelist[i_31].id_length = edgelist[i_31].id_length / 12;
                }
                edgelist[i_31].smooth = false;
            }
        }
        //極小ノードをクラスターに分類していく
        ...
        var clusterNo = 0;
        for (var i_32 = 0; i_32 < notInterestedNodes.length; i_32++) {
            if (clusterNumber[i_32] == -1) {
                clusterNumber[i_32] = clusterNo;
                while (true) {
                    var bool = false;
                    for (var j_15 = 0; j_15 < notInterestedEdges.length; j_15++) {
                        var dot1_1 = notInterestedEdges[j_15].dot1;
                        var dot2_1 = notInterestedEdges[j_15].dot2;
                        ...
                        if (cn1 == clusterNo && cn2 != clusterNo) {
                            ...
                            clusterEdgeNumber[j_15] = clusterNo;
                            bool = true;
                        }
        ...
        for (var i_33 = 0; i_33 < clusterNo; i_33++) {
            var clusterNodeArray = new Array();
            for (var j_16 = 0; j_16 < clusterNumber.length; j_16++) {
                if (clusterNumber[j_16] == i_33) {
                    clusterNodeArray.push(notInterestedNodes[j_16]);
                }
            }
            notInterestNodeCluster.push(clusterNodeArray);
            var clusterEdgeArray = new Array();
            for (var j_17 = 0; j_17 < clusterEdgeNumber.length; j_17++) {
                if (clusterEdgeNumber[j_17] == i_33) {
                    clusterEdgeArray.push(notInterestedEdges[j_17]);
                }
            }
            notInterestEdgeCluster.push(clusterEdgeArray);
        }
        for (var i_34 = 0; i_34 < notInterestEdgeCluster.length; i_34++) {
            var notRootNode = new Array(notInterestNodeCluster[i_34].length);
            notRootNode.fill(true);
            for (var j_18 = 0; j_18 < notInterestEdgeCluster[i_34].length; j_18++) {
                var toNode = notInterestEdgeCluster[i_34][j_18].dot2;
                notRootNode[notInterestNodeCluster[i_34].indexOf(toNode)] = false;
            }
            if (notRootNode.indexOf(true) == -1) { //根のノードがクラスターになかった場合
                notInterestNodeClusterSort.push(notInterestNodeCluster[i_34]);
            }
            else {
                var root = notInterestNodeCluster[i_34][notRootNode.indexOf(true)]; //根のノード
                var sortCluster = new Array();
                edgeAngleSort(root, notInterestEdgeCluster[i_34], sortCluster);
                notInterestNodeClusterSort.push(sortCluster);
                function edgeAngleSort(node, edgeCluster, sortCluster) {
                    var edgeArray = new Array();
                    for (var i_35 = 0; i_35 < edgeCluster.length; i_35++) {
                        if (edgeCluster[i_35].dot1 == node)
                            edgeArray.push(edgeCluster[i_35]);
                    }
                    if (edgeArray.length == 0)
                        sortCluster.push(node);
                    else {
                        for (var i_36 = 0; i_36 < edgeArray.length; i_36++) { //エッジを理想角度の大きさでバブルソートする
                            for (var j_19 = 1; j_19 < edgeArray.length - i_36; j_19++) {
                                if (edgeArray[j_19].id_angle > edgeArray[j_19 - 1].id_angle) {
                                    var temp = edgeArray[j_19];
                                    edgeArray[j_19] = edgeArray[j_19 - 1];
                                    edgeArray[j_19 - 1] = temp;
                                }
                            }
                        }
                        var flag = false;
                        for (var i_37 = 0; i_37 < edgeArray.length; i_37++) {
                            if (edgeArray[i_37].id_angle <= 90 && !flag) {
                                sortCluster.push(node);
                                flag = true;
                            }
                            edgeAngleSort(edgeArray[i_37].dot2, edgeCluster, sortCluster);
                        }
                    }
                }
            }
        }
        for (var i_38 = 0; i_38 < notInterestNodeClusterSort.length; i_38++) {
            var nodeCluster = notInterestNodeClusterSort[i_38];
            var fromClusterEdges = new Array();
            for (var j_20 = 0; j_20 < nodeCluster.length; j_20++) {
                var node = nodeCluster[j_20];
            ...
            for (var j_21 = 0; j_21 < fromClusterEdges.length; j_21++) {
                fromClusterEdges[j_21].id_angle = 120 - 60 / (fromClusterEdges.length * 2) * (2 * j_21 + 1);
            }
        }
    ...
    function makeCluster(nodelist, edgelist, clusters, notInterestedNodeClusterSort) {
        ...
        for (var i_39 = 0; i_39 < notInterestedNodeClusterSort.length; i_39++) {
            var toClusterEdges = new Array();
            for (var j_22 = 0; j_22 < notInterestedNodeClusterSort[i_39].length; j_22++) {
                for (var k = 0; k < edgelist.length; k++) {
                    if (edgelist[k].dot2 == notInterestedNodeClusterSort[i_39][j_22] && edgelist[k].dot1.interested)
                        toClusterEdges.push(edgelist[k]);
                }
            }
            if (toClusterEdges.length == 0) {
                var cluster = new FiFA_Cluster_Node(null, notInterestedNodeClusterSort[i_39]);
                var jnum = Math.floor((notInterestedNodeClusterSort[i_39].length - 1) / 2);
                var centerNode = notInterestedNodeClusterSort[i_39][jnum];
                cluster.init(centerNode.x, centerNode.y);
                clusters.push(cluster);
            }
            else {
                var mainDot = toClusterEdges[0].dot1;
                var cluster = new FiFA_Cluster_Node(mainDot, notInterestedNodeClusterSort[i_39]);
                cluster.init(mainDot.x, mainDot.y);
                clusters.push(cluster);
                usedInterestNodes.push(mainDot);
            }
        }
        for (var i_40 = 0; i_40 < nodelist.length; i_40++) {
            if (usedInterestNodes.indexOf(nodelist[i_40]) == -1 && nodelist[i_40].interested) {
                var cluster = new FiFA_Cluster_Node(nodelist[i_40], []);
                cluster.init(nodelist[i_40].x, nodelist[i_40].y);
                clusters.push(cluster);
            }
        }
    }
    ...
    function makeClusterEdge(edgelist, clusteredgelist) {
        for (var i_41 = 0; i_41 < edgelist.length; i_41++) {
            var cluster1 = edgelist[i_41].dot1.cluster;
            var cluster2 = edgelist[i_41].dot2.cluster;
            if (cluster1 != cluster2) { //異なるクラスターを結んでいるエッジ
                var clusteredge = new FiFA_Cluster_Edge(cluster1, cluster2, edgelist[i_41]);
                clusteredgelist.push(clusteredge);
                cluster1.f_cluster_edges.push(clusteredge);
            }
            else {
                cluster1.f_include_edges.push(edgelist[i_41]);
            }
        }
        //重複するエッジを削除する
        for (var i_42 = clusteredgelist.length - 1; i_42 > 0; i_42--) {
            var clusteredge1 = clusteredgelist[i_42];
            for (var j_23 = 0; j_23 < i_42; j_23++) {
                var clusteredge2 = clusteredgelist[j_23];
                if (clusteredge1.cluster1 == clusteredge2.cluster1 && clusteredge1.cluster2 == clusteredge2.cluster2) {
                    clusteredgelist.splice(i_42, 1);
                }
    ...
    function calculateLocationWithForceDirectedMethod(graph, nodelist, edgelist, clusternodelist, clusteredgelist, attentionNodes) {
        ...
        do {
            for (var i_43 = 0; i_43 < CLUSTERNUM; i_43++) {
                clusternodelist[i_43].init(Math.floor(Math.random() * WIDTH), Math.floor(Math.random() * HEIGHT));
                clusternodelist[i_43].sub_init(30, 30);
            }
        ...
        for (var i_44 = 0; i_44 < CEDGENUM; i_44++) {
            var c1 = clusternodelist.indexOf(clusteredgelist[i_44].cluster1);
            var c2 = clusternodelist.indexOf(clusteredgelist[i_44].cluster2);
        ...
        for (var i_45 = 0; i_45 < CLUSTERNUM; i_45++) {
            var nodeNum = clusternodelist[i_45].quantity();
            ad_matrixes[i_45] = new Array(nodeNum * nodeNum);
            ad_matrixes[i_45].fill(false);
            for (var j_24 = 0; j_24 < clusternodelist[i_45].f_include_edges.length; j_24++) {
                var dot1_2 = clusternodelist[i_45].f_include_edges[j_24].dot1;
                var dot2_2 = clusternodelist[i_45].f_include_edges[j_24].dot2;
                var n1 = (dot1_2 == clusternodelist[i_45].main) ? 0 : clusternodelist[i_45].sub.indexOf(dot1_2) + 1;
                var n2 = (dot2_2 == clusternodelist[i_45].main) ? 0 : clusternodelist[i_45].sub.indexOf(dot2_2) + 1;
                ad_matrixes[i_45][n1 * nodeNum + n2] = true;
                ad_matrixes[i_45][n2 * nodeNum + n1] = true;
            }
        }
        ...
        function cluster_force_move() {
            ...
            for (var i_46 = 0; i_46 < CLUSTERNUM; i_46++) {
                var dx = clusternodelist[i_46].dx;
                var dy = clusternodelist[i_46].dy;
                var disp = Math.sqrt(dx * dx + dy * dy);
                if (disp != 0) {
                    var d = Math.min(disp, ct) / disp;
                    clusternodelist[i_46].move(dx * d, dy * d);
                }
            }
            //温度パラメータを下げていく
            ct -= cdt;
        ...
        function force_move() {
            ...
            for (var i_47 = 0; i_47 < CLUSTERNUM; i_47++) {
                //mainの移動
                if (clusternodelist[i_47].main != null) {
                    var dx = clusternodelist[i_47].main.dx;
                    var dy = clusternodelist[i_47].main.dy;
                    var disp = Math.sqrt(dx * dx + dy * dy);
                    if (disp != 0) {
                        var d = Math.min(disp, t) / disp;
                        clusternodelist[i_47].main.move(dx * d, dy * d);
                    }
                }
                //subの移動
                for (var j_25 = 0; j_25 < clusternodelist[i_47].sub.length; j_25++) {
                    var dx = clusternodelist[i_47].sub[j_25].dx;
                    var dy = clusternodelist[i_47].sub[j_25].dy;
                    var disp = Math.sqrt(dx * dx + dy * dy);
                    if (disp != 0) {
                        var d = Math.min(disp, t) / disp;
                        clusternodelist[i_47].sub[j_25].move(dx * d, dy * d);
                    }
                }
                //クラスター内のノード座標をクラスター座標に合わせる
                clusternodelist[i_47].move_only_node();
            }
            //温度パラメータを下げていく
            t -= dt;
        }
        ...
        function cluster_focus_calculate(clusternodelist) {
            //各点の速度・力ベクトルを0に初期化
            for (var i_48 = 0; i_48 < CLUSTERNUM; i_48++) {
                clusternodelist[i_48].focus_init();
            }
            //エッジの端点に働く角度力とスプリング力を計算
            for (var i_49 = 0; i_49 < clusteredgelist.length; i_49++) {
                var dx = clusteredgelist[i_49].cluster2.x - clusteredgelist[i_49].cluster1.x;
                var dy = clusteredgelist[i_49].cluster2.y - clusteredgelist[i_49].cluster1.y;
                var delta = Math.sqrt(dx * dx + dy * dy);
                if (delta != 0) {
                    var d1size = clusteredgelist[i_49].cluster1.size;
                    var d2size = clusteredgelist[i_49].cluster2.size;
                    if (clusteredgelist[i_49].underforce == true) {
                        //各点の角度に基づいて働く力を計算
                        var d = clusteredgelist[i_49].angle() - clusteredgelist[i_49].id_angle; //弧度法から度数法に変更
                        var ddx = void 0;
                        var ddy = void 0;
                        var krad = clusteredgelist[i_49].krad * d2size * d1size / (NODESIZE * NODESIZE);
                        ...
                        clusteredgelist[i_49].cluster1.fmx += -ddx;
                        clusteredgelist[i_49].cluster1.fmy += -ddy;
                        clusteredgelist[i_49].cluster2.fmx += ddx;
                        clusteredgelist[i_49].cluster2.fmy += ddy;
                    }
                    //２点が辺で繋がっている場合はスプリング力を計算
                    var ds = f_s(delta, cs, clusteredgelist[i_49].id_length) / delta * d2size * d1size / (NODESIZE * NODESIZE);
                    var ddsx = dx * ds;
                    var ddsy = dy * ds;
                    clusteredgelist[i_49].cluster2.fax -= ddsx;
                    clusteredgelist[i_49].cluster2.fay -= ddsy;
                    clusteredgelist[i_49].cluster1.fax += ddsx;
                    clusteredgelist[i_49].cluster1.fay += ddsy;
                }
            }
            //各点の斥力を計算
            for (var i_50 = 0; i_50 < CLUSTERNUM; i_50++) {
                for (var j_26 = i_50 + 1; j_26 < CLUSTERNUM; j_26++) {
                    if (cluster_admx[i_50 * CLUSTERNUM + j_26] == false) {
                        var dx = clusternodelist[i_50].x - clusternodelist[j_26].x;
                        var dy = clusternodelist[i_50].y - clusternodelist[j_26].y;
                        var delta = Math.sqrt(dx * dx + dy * dy);
                        if (delta != 0 && delta < 800) {
                            var d1size = clusternodelist[i_50].size;
                            var d2size = clusternodelist[j_26].size;
                            var d = f_r(delta, cr) / delta * d1size * d2size / (NODESIZE * NODESIZE);
                            clusternodelist[i_50].frx += dx * d;
                            clusternodelist[i_50].fry += dy * d;
                            clusternodelist[j_26].frx -= dx * d;
                            clusternodelist[j_26].fry -= dy * d;
                        }
                    }
                }
            }
            //力ベクトルから速度を求める
            for (var i_51 = 0; i_51 < CLUSTERNUM; i_51++) {
                clusternodelist[i_51].init_velocity();
            }
        }
        //各クラスター内でそれぞれのノード間に働く引力・斥力を計算する
        function focus_calculate(clusternodelist, nodelist) {
            //各点の速度・力ベクトルを0に初期化
            for (var i_52 = 0; i_52 < nodelist.length; i_52++) {
                nodelist[i_52].focus_init();
            }
            for (var i_53 = 0; i_53 < CLUSTERNUM; i_53++) {
                var cluster = clusternodelist[i_53];
                ...
                //エッジの端点に働く角度力とスプリング力を計算
                for (var j_27 = 0; j_27 < smalledgelist.length; j_27++) {
                    var dx = smalledgelist[j_27].dot2.x - smalledgelist[j_27].dot1.x;
                    var dy = smalledgelist[j_27].dot2.y - smalledgelist[j_27].dot1.y;
                    var delta = Math.sqrt(dx * dx + dy * dy);
                    if (delta != 0) {
                        if (smalledgelist[j_27].underforce == true) {
                            //各点の角度に基づいて働く力を計算
                            var d = smalledgelist[j_27].angle() - smalledgelist[j_27].id_angle; //弧度法から度数法に変更
                            var ddx = void 0;
                            var ddy = void 0;
                            var krad = smalledgelist[j_27].krad / 2;
                            var ex = krad * dy / delta; //角度に関する力の基本ベクトル（元のベクトルを負の方向に90度回転）
                            var ey = -krad * dx / delta; //角度に関する力の基本ベクトル（元のベクトルを負の方向に90度回転）
                            if (Math.abs(d) <= 180) {
                                ddx = d * Math.abs(d) * ex;
                            ...
                            smalledgelist[j_27].dot1.fmx += -ddx;
                            smalledgelist[j_27].dot1.fmy += -ddy;
                            smalledgelist[j_27].dot2.fmx += ddx;
                            smalledgelist[j_27].dot2.fmy += ddy;
                        }
                        //２点が辺で繋がっている場合はスプリング力を計算
                        var ds = f_s(delta, cs, smalledgelist[j_27].id_length) / delta / 10;
                        var ddsx = dx * ds;
                        var ddsy = dy * ds;
                        smalledgelist[j_27].dot2.fax -= ddsx;
                        smalledgelist[j_27].dot2.fay -= ddsy;
                        smalledgelist[j_27].dot1.fax += ddsx;
                        smalledgelist[j_27].dot1.fay += ddsy;
                    }
                }
                //各点の斥力を計算
                for (var j_28 = isMain; j_28 < nodeNum; j_28++) {
                    for (var k = j_28 + 1; k < nodeNum; k++) {
                        if (ad_matrixes[i_53][j_28 * nodeNum + k] == false) {
                            var dx = includelist[j_28].x - includelist[k].x;
                            var dy = includelist[j_28].y - includelist[k].y;
                            var delta = Math.sqrt(dx * dx + dy * dy);
                            if (delta != 0 && delta < 800) {
                                var d = f_r(delta, cr) / delta / 160;
                                includelist[j_28].frx += dx * d;
                                includelist[j_28].fry += dy * d;
            ...
            //力ベクトルから速度を求める
            for (var i_54 = 0; i_54 < nodelist.length; i_54++) {
                nodelist[i_54].init_velocity();
            }
        }
        ...
        function stopCalculate() {
            ...
            for (var i_55 = 0; i_55 < UsingObjectIDs.length; i_55++) {
                graph.setLocation(UsingObjectIDs[i_55], nodelist[i_55].x, nodelist[i_55].y); //ノードの座標
                graph.setDistance(UsingObjectIDs[i_55], nodelist[i_55].route_length); //ノードの注目点からの距離（デバッグ用）
                graph.setSize(UsingObjectIDs[i_55], nodelist[i_55].size); //ノードのサイズ
                graph.setColor(UsingObjectIDs[i_55], nodelist[i_55].color);
                var label = "<b>" + nodelist[i_55].cls + "</b>";
                for (var x in nodelist[i_55].literals) {
                    if (nodelist[i_55].literals.hasOwnProperty(x)) {
                        label = label + "\n <i> " + x + "</i>: " + nodelist[i_55].literals[x];
                    }
                }
                graph.setLabel(UsingObjectIDs[i_55], label);
            }
            for (var i_56 = 0; i_56 < edgelist.length; i_56++) {
                var edge = edgelist[i_56];
                var dot1ID = UsingObjectIDs[nodelist.indexOf(edge.dot1)];
                var dot2ID = UsingObjectIDs[nodelist.indexOf(edge.dot2)];
            ...
            for (var i_57 = 0; i_57 < greenEdges.length; i_57++) {
                if (!nodelist[UsingObjectIDs.indexOf(greenEdges[i_57].to)].interested) {
                    var toNode = nodelist[UsingObjectIDs.indexOf(greenEdges[i_57].to)];
                    ...
                    //エッジの長さ
                    graph.setVariableEdgeLength(greenEdges[i_57].to, STANDARD_EDGELENGTH / 100);
                }
            }
            if (attentionNodes.length > 0) {
                for (var i_58 = 0; i_58 < attentionNodes.length; i_58++) {
                    var node = nodelist[UsingObjectIDs.indexOf(attentionNodes[i_58])];
                    var edgefontSize = (node.size - NODEMINSIZE) * 10 / (NODEMAXSIZE - NODEMINSIZE) + STANDARD_EDGEFONTSIZE;
                    graph.setVariableEdgeLabelSize(attentionNodes[i_58], edgefontSize); //緑エッジのラベルのサイズ
                }
            }
        }
        ...
        function move_near_center(nodelist, clusternodelist) {
            ...
            for (var i_59 = 0; i_59 < nodelist.length; i_59++) {
                if (darray[i_59] != -1) {
                    var ary = new Array();
                    ary.push(i_59);
                    darray[i_59] = -1;
                    for (j = i_59 + 1; j < nodelist.length; j++) {
                        var n1 = clusternodelist.indexOf(nodelist[i_59].cluster);
            ...
            var Rectangle_Nodes = /** @class */ (function () {
                ...
                Rectangle_Nodes.prototype.calculation = function () {
                    ...
                    for (var i_60 = 1; i_60 < this.nodeNumber; i_60++) {
                        if (nodelist[this.nodeArray[i_60]].x < left) {
                            left = nodelist[this.nodeArray[i_60]].x;
                        }
                        if (nodelist[this.nodeArray[i_60]].x > right) {
                            right = nodelist[this.nodeArray[i_60]].x;
                        }
                        if (nodelist[this.nodeArray[i_60]].y < up) {
                            up = nodelist[this.nodeArray[i_60]].y;
                        }
                        if (nodelist[this.nodeArray[i_60]].y > down) {
                            down = nodelist[this.nodeArray[i_60]].y;
                        }
                    }
                    this.leftX = left;
                    this.rightX = right;
                ...
                Rectangle_Nodes.prototype.moveX = function (mx) {
                    for (var i_61 = 0; i_61 < this.nodeNumber; i_61++) {
                        nodelist[this.nodeArray[i_61]].x += mx;
                    }
                };
                Rectangle_Nodes.prototype.moveY = function (my) {
                    for (var i_62 = 0; i_62 < this.nodeNumber; i_62++) {
                        nodelist[this.nodeArray[i_62]].y += my;
                    }
                };
            ...
            for (var i_63 = 0; i_63 < groupArray.length; i_63++) {
                groupRectangle[i_63] = new Rectangle_Nodes(groupArray[i_63]);
            }
            var t = T;
            var dt = T / 1000;
            while (true) {
                ...
                for (var i_64 = 0; i_64 < groupRectangle.length; i_64++) {
                    groupRectangle[i_64].resetPower();
                    for (var j_29 = 0; j_29 < groupRectangle.length; j_29++) {
                        if (i_64 != j_29) {
                            var dx = groupRectangle[j_29].centerX() - groupRectangle[i_64].centerX();
                            var dy = groupRectangle[j_29].centerY() - groupRectangle[i_64].centerY();
                            var delta = Math.sqrt(dx * dx + dy * dy);
                            var ideal_length = (groupRectangle[j_29].diagonal_length() + groupRectangle[i_64].diagonal_length()) * 1.1;
                            var spower = f_s(delta, CS, ideal_length);
                            if (delta != 0) {
                                groupRectangle[i_64].powerX += dx * spower / delta;
                                groupRectangle[i_64].powerY += dy * spower / delta;
                            }
                        }
                    }
                }
                //計算した力を元にグループ単位で移動させていく
                for (var i_65 = 0; i_65 < groupRectangle.length; i_65++) {
                    var dx = groupRectangle[i_65].powerX;
                    var dy = groupRectangle[i_65].powerY;
                    var disp = Math.sqrt(dx * dx + dy * dy);
                    if (disp != 0) {
                        var d = Math.min(disp, t) / disp;
                        groupRectangle[i_65].moveX(dx * d);
                        groupRectangle[i_65].moveY(dy * d);
                    }
                    groupRectangle[i_65].calculation();
                }
                t -= dt;
                if (t <= 0)
                    break;
            }
            for (var i_66 = 0; i_66 < clusternodelist.length; i_66++) {
                if (clusternodelist[i_66].main != null) {
                    clusternodelist[i_66].x = clusternodelist[i_66].main.x;
                    clusternodelist[i_66].y = clusternodelist[i_66].main.y;
                }
            }
    ...
    function sameNode_exists(nodelist) {
        var bool = false;
        for (var i_67 = 0; i_67 < nodelist.length - 1; i_67++) {
            for (var j_30 = i_67 + 1; j_30 < nodelist.length; j_30++) {
                bool = bool || (nodelist[i_67].x == nodelist[j_30].x && nodelist[i_67].y == nodelist[j_30].y);
            }
        }
        return bool;
    }
    ...
    function FloydWarshall_cluster(clusternodelist, clusteredgelist, mtx) {
        ...
        for (var i_68 = 0; i_68 < nodeNum; i_68++) {
            for (var j_31 = 0; j_31 < nodeNum; j_31++) {
                mtx[i_68 * nodeNum + j_31] = nodeNum;
            }
            mtx[i_68 * nodeNum + i_68] = 0;
        }
        for (var i_69 = 0; i_69 < clusteredgelist.length; i_69++) {
            var one = clusternodelist.indexOf(clusteredgelist[i_69].cluster1);
            var two = clusternodelist.indexOf(clusteredgelist[i_69].cluster2);
            mtx[one * nodeNum + two] = 1;
            mtx[two * nodeNum + one] = 1;
        }
        for (var k = 0; k < nodeNum; k++) {
            for (var i_70 = 0; i_70 < nodeNum; i_70++) {
                for (var j_32 = 0; j_32 < nodeNum; j_32++) {
                    if (mtx[i_70 * nodeNum + j_32] > mtx[i_70 * nodeNum + k] + mtx[k * nodeNum + j_32]) {
                        mtx[i_70 * nodeNum + j_32] = mtx[i_70 * nodeNum + k] + mtx[k * nodeNum + j_32];
                    }
                }
            }
        }
    ...
    function center_of_gravity(nodelist, clusternodelist, width, height) {
        if (nodelist.length == 0)
            ...
            var cx = 0;
            var cy = 0;
            for (var i_71 = 0; i_71 < nodelist.length; i_71++) {
                cx += nodelist[i_71].x;
                cy += nodelist[i_71].y;
            }
            ...
            var dx = width / 2 - cx;
            var dy = height / 2 - cy;
            for (var i_72 = 0; i_72 < clusternodelist.length; i_72++) {
                clusternodelist[i_72].move(dx, dy);
            }
        }
    }
    //注目ノードの特定
    function attentionNodesInit(graph) {
        ...
        if ( /*bool*/true) {
            for (var i_74 = 0; i_74 < greenEdges.length; i_74++) {
                //ローカル変数の指すノードを拡大表示する
                if (greenEdges[i_74].label != "this" && global_letiables.indexOf(greenEdges[i_74].label) == -1) {
                    attentionNodes.push(greenEdges[i_74].to);
                }
            }
        }
